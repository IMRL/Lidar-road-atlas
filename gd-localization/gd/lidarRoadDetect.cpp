...
#endif
{
    cv::Mat3f imagery;
    cv::Mat1b valid;
#if defined(CONF_VLP16) || defined(CONF_VLP16_SIMULATOR)
    std::tie(imagery, valid) = make_imagery(cloud);
#elif defined(CONF_VLP64) || defined(CONF_OUSTER64)
...
std::tuple<cv::Mat3f, cv::Mat1b> LidarRoadDetect::make_imagery_downsample(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
{
    ...
    for (int i = 0; i < Range_Num; i++)
    {
        ...
        imagery.row(i * Down_sample).copyTo(imagery_down.row(i));
        valid.row(i * Down_sample).copyTo(valid_down.row(i));
    }
