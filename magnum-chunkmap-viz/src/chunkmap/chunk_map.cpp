#include "chunk_map.h"
#include "simple_probability_values.h"
...
pcl::PointCloud<pcl::PointXYZ>::Ptr ChunkMap::generateObstacleCloud(float baseHeight, float segHeight)
{
                ...
                // if (layer.observe(r, c))
                {
                    ...
                    for (int b = 0; b <= ObstacleTypeDesc_max_height(obstacle_segments, obstacle_bits); b++)
                        if ((value & (1 << b)) != 0)
                        {
                            ...
                            p.x = x;
                            p.y = y;
                            p.z = z + b * segHeight + baseHeight; // + 0.5;
                            cloud->push_back(p);
                        }
                }
    }
...
template <int N = 0>
std::vector<uint8_t> ObstacleTypeProbDesc_at_impl(const ObstacleFeature::ObstacleDescIndex &key, const cv::Mat &mat, int r, int c)
{
    constexpr auto item = std::get<N>(ObstacleFeature::ObstacleDescList);
    if (key == item)
    {
        ...
        if constexpr (item.second > ObstacleFeature::Bit1)
        {
            ...
            for (int i=0;i<item.first;i++) {
                ret.push_back((uint8_t)elem(i * item.second, item.second));
            }
            // for (int i = item.first - 1; i >= 0; i--)
            // {
            //     ret <<= 1;
        ...
        return ObstacleTypeProbDesc_at_impl<N + 1>(key, mat, r, c);
}
template <>
std::vector<uint8_t> ObstacleTypeProbDesc_at_impl<ObstacleFeature::ObstacleDescListLength>(const ObstacleFeature::ObstacleDescIndex &, const cv::Mat &, int, int)
{
    return {};
}
std::vector<uint8_t> ObstacleTypeProbDesc_at(const ObstacleFeature::Segments &seg, const ObstacleFeature::Bits &bit, const cv::Mat &mat, int r, int c)
{
    return ObstacleTypeProbDesc_at_impl({seg, bit}, mat, r, c);
}

pcl::PointCloud<pcl::PointXYZI>::Ptr ChunkMap::generateObstacleCloudProb()
{
        ...
        for (auto &layer : chunk.second.getLayers())
            for (int r = 0; r < chunk_size; r++)
                ...
                // if (layer.observe(r, c))
                {
                    ...
                    float z = layer.elevation(r, c);
                    // int32_t value = layer.obstacle_info.at<int32_t>(r, c);
                    auto value = ObstacleTypeProbDesc_at(obstacle_segments, obstacle_bits, layer.obstacle_info, r, c);
                    for (int b = 0; b <= ObstacleTypeDesc_max_height(obstacle_segments, obstacle_bits); b++)
                    {
                        pcl::PointXYZI p;
                        p.x = x;
