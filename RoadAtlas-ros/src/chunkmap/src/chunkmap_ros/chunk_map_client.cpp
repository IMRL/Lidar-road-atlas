    ...
    : ChunkMap({}), fn(fn)
{
    ...
    if (ros::service::call(base_topic + "/get_info", mapInfo))
    {
        ...
        obstacle_segments = ObstacleFeature::Segments(mapInfo.response.ChunkMapInfo.obstacle_bits >> 16);
        obstacle_bits = ObstacleFeature::Bits(mapInfo.response.ChunkMapInfo.obstacle_bits & 0xffff);
        // init_map();
