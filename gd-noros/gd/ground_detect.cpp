...
class GroundDetectImpl : public GroundDetect
{
    ...
    {
        cv::Mat_<uint16_t> submap_value = cv::Mat_<uint16_t>::ones(int(2 * config_.submap_radius / resolution), int(2 * config_.submap_radius / resolution)) * inital_map_value;
    ...
    {
        ObstacleTypeDesc::Type submap_value = ObstacleTypeDesc::Type::zeros(int(2 * config_.submap_radius / config_.resolution), int(2 * config_.submap_radius / config_.resolution));
        {
            cv::Mat1f xyz[3];
            cv::split(imagery, xyz);
    ...
    {
        {
            ...
            // for mulran
            int th1 = 60 * LidarRoadDetect::H_Ang_Num / 360; // 90
            int th2 = 300 * LidarRoadDetect::H_Ang_Num / 360; // 270
        ...
        mapOffset = {int(config_.submap_radius / config_.resolution), int(config_.submap_radius / config_.resolution)};
    }
